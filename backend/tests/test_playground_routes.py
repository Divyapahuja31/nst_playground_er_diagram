...
def test_teacher_and_student_playground_isolation(client, teacher_user, student_user):
    solution_diagram = {
    ...
    client.put(f"/questions/{q_id}", json={"is_published": True}, headers=teacher_user["headers"])

    # 2. Teacher opens playground -> is_solution MUST be True
    t_pg = client.get(f"/questions/{q_id}/playground", headers=teacher_user["headers"]).json()
    assert t_pg["is_solution"] is True

    # 3. Student opens playground -> is_solution MUST be False
    s_pg = client.get(f"/questions/{q_id}/playground", headers=student_user["headers"]).json()
    assert s_pg["is_solution"] is False


def test_reviewing_teacher_solution_reveal(client, teacher_user, reviewer_teacher, student_user):
    solution_diagram = {
    ...
    rev_res = client.get(f"/questions/{q_id}/solution", headers=reviewer_teacher["headers"])
    assert rev_res.status_code == 200
    assert rev_res.json()["solution"] == solution_diagram

    # 3. Student attempts to reveal official solution -> Forbidden (403)
    stu_res = client.get(f"/questions/{q_id}/solution", headers=student_user["headers"])
    assert stu_res.status_code == 403


def test_student_submit_solution_evaluation(client, teacher_user, student_user):
    diagram = {
    ...
    client.get(f"/questions/{q_id}/playground", headers=student_user["headers"])
    client.put(
    ...
    sub_res = client.post(f"/questions/{q_id}/submit", json={}, headers=student_user["headers"])
