...
def test_create_question_by_teacher(client, teacher_user):
    solution_diagram = {
...
def test_student_cannot_view_unpublished_question(client, teacher_user, student_user):
    ...
    stu_res = client.get(f"/questions/{q_id}", headers=student_user["headers"])
    assert stu_res.status_code == 403


def test_student_can_view_published_question_without_solution(client, teacher_user, student_user):
    ...
    pub_res = client.put(f"/questions/{q_id}", json={"is_published": True}, headers=teacher_user["headers"])
